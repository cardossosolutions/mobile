      ...
      <View style={styles.cardContent}>
        ...
        {provider.observation && (
          <View style={styles.infoRow}>
            ...
            <Text style={styles.infoText} numberOfLines={2}>{provider.observation}</Text>
          </View>
        )}
      </View>
...
const ServiceProviderManagementScreen: React.FC = () => {
  const { serviceProviders, loadServiceProviders, deleteServiceProvider } = useData();
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      loadServiceProviders(1, searchTerm);
    }, 500);
  ...
  const handleRefresh = () => {
    setRefreshing(true);
    loadServiceProviders(1, searchTerm).finally(() => setRefreshing(false));
  };
  ...
  const handleDelete = (provider: ServiceProvider) => {
    Alert.alert(
      'Confirmar Exclusão',
      `Tem certeza que deseja excluir o prestador "${provider.name}"?`,
      [
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <Text style={styles.headerSubtitle}>
          {serviceProviders.length} {serviceProviders.length === 1 ? 'prestador' : 'prestadores'}
        </Text>
      </LinearGradient>
